...
class CnnTvNetAgent(nn.Module):
    ...
    def load(dirname, envs, base_dir, map_location=None):
        ...
        if base_dir == dirname:
            print("loading base actor and encoder")
            ...
            print("loading base actor and encoder and task vector")
...
class CnnTv2NetAgent(nn.Module):
    ...
    def load(dirname, envs, base_dir, map_location=None):
        ...
        if base_dir == dirname:
            ...
            print("loading base encoder and task vector")
            # load the state dict of the vector
            encoder_vector = torch.load(f"{dirname}/encoder.pt", map_location=map_location)
...
class CnnTv3NetAgent(nn.Module):
    def __init__(self, envs, prevs_paths=[], map_location=None):
        super().__init__()
        self.hidden_dim = 512
        self.is_first = len(prevs_paths) == 0
        ...
        if not self.is_first:
            thetas = [
                torch.load(f"{p}/theta1.pt", map_location=map_location)
                for p in prevs_paths[1:]
            ]
            self.actor = nn.Sequential(
                IncreParamLinearLayer(nn.Linear(self.hidden_dim, self.hidden_dim), vectors=thetas),
                nn.ReLU(),
                layer_init(nn.Linear(self.hidden_dim, envs.single_action_space.n), std=0.01),
            )
